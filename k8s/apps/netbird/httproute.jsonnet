    ...
    rules: [
      {
        matches: [
          ...
          { path: { type: 'PathPrefix', value: '/management.ProxyService/' } },
        ],
        backendRefs: [{ name: app.name + '-server', port: 81 }],
      },
      {
        matches: [
        ...
        backendRefs: [{ name: app.name + '-server', port: 80 }],
      },
      { backendRefs: [{ name: app.name + '-dashboard', port: 80 }] },
    ],
  },
}
