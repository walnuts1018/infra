function(app)
  ...
  local sa = (import '../../sa.libsonnet')(app);
  {
    ...
    kind: 'Deployment',
    metadata: {
      name: app.name + '-stack-timeline-worker',
      namespace: app.namespace,
      labels: labels(app.name + '-stack-timeline-worker'),
    },
    spec: {
      replicas: 0,
      selector: {
        matchLabels: labels(app.name + '-stack-timeline-worker'),
      },
      template: {
        metadata: {
          labels: labels(app.name + '-stack-timeline-worker'),
        ...
        spec: {
          ...
          containers: [
            (import '../../../../container.libsonnet') {
              ...
              image: 'ghcr.io/walnuts1018/picca/stack-timeline-worker:v0.0.95@sha256:d1251d59a1cca1a8e0c1f4d85d9d0472d66ef835e231d726f5e4da067df9d5bc',
              ...
              env: commonEnv + s3Irsa.env + scyllaTls.env + plans.env + [
                {
                  ...
                  value: 'picca-stack-timeline-worker',
                },
              ],
              resources: {
                requests: {
