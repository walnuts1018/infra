function(app)
  ...
  local sa = (import '../../sa.libsonnet')(app);
  {
    ...
    kind: 'Deployment',
    metadata: {
      name: app.name + '-empty-trash-worker',
      namespace: app.namespace,
      labels: labels(app.name + '-empty-trash-worker'),
    },
    spec: {
      replicas: 0,
      selector: {
        matchLabels: labels(app.name + '-empty-trash-worker'),
      },
      template: {
        metadata: {
          labels: labels(app.name + '-empty-trash-worker'),
        ...
        spec: {
          ...
          containers: [
            (import '../../../../container.libsonnet') {
              name: 'empty-trash-worker',
              image: 'ghcr.io/walnuts1018/picca/empty-trash-worker:v0.0.95',
              ...
              env: commonEnv + s3Irsa.env + scyllaTls.env + plans.env + [
                {
                  ...
                  value: 'picca-empty-trash-worker',
                },
              ],
              resources: {
                requests: {
