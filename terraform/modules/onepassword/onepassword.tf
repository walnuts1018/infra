...
resource "random_id" "picca_dev_imgproxy_key" {
  byte_length = 32
}

resource "random_id" "picca_dev_imgproxy_salt" {
  byte_length = 32
}

resource "random_password" "picca_dev_redis_password" {
  length  = 32
  special = false
}

resource "random_password" "picca_dev_rabbitmq_password" {
  length  = 32
  special = false
}
